  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-xl font-semibold text-foreground">Gestão do Blog</h1>
          <p className="text-sm text-muted-foreground mt-0.5">{posts?.length ?? 0} artigos publicados</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[hsl(40,10%,85%)] bg-[hsl(40,43%,96%)]">
                ...
                <th className="text-left px-4 py-3 text-xs tracking-widest uppercase text-muted-foreground font-normal hidden md:table-cell">Excerto</th>
            ...
            <tbody>
              {posts.map((post) => (
                <tr key={post.id} className="border-b border-[hsl(40,10%,85%)] last:border-0" data-testid={`row-post-${post.id}`}>
                  <td className="px-4 py-3 font-medium">{post.titulo}</td>
                  <td className="px-4 py-3 text-muted-foreground hidden md:table-cell text-xs max-w-xs truncate">
                    {post.conteudo.length > 80 ? post.conteudo.slice(0, 80) + "..." : post.conteudo}
                  </td>
                  <td className="px-4 py-3 text-muted-foreground text-xs">{formatDate(post.dataPublicacao)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(post)} data-testid={`button-edit-post-${post.id}`} className="p-1.5 text-muted-foreground hover:text-foreground transition-colors"><Pencil size={14} /></button>
                      <button onClick={() => setDeleting(post)} data-testid={`button-delete-post-${post.id}`} className="p-1.5 text-muted-foreground hover:text-red-600 transition-colors"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="titulo" render={({ field }) => (
                <FormItem><FormLabel className="text-xs tracking-widest uppercase">Título *</FormLabel><FormControl><Input {...field} data-testid="input-titulo-post" className="rounded-none" /></FormControl><FormMessage /></FormItem>
              )} />
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      ...
                      className="rounded-none resize-none font-mono text-sm"
                      rows={14}
              ...
              <DialogFooter className="pt-2">
                ...
                <Button type="submit" disabled={createPost.isPending || updatePost.isPending} data-testid="button-submit-post" className="rounded-none text-xs tracking-widest uppercase bg-foreground hover:bg-foreground/90">
